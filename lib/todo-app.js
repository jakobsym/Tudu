...
function subscriptions(signal){
    var ENTER = 'Enter'; // if user presses 'Enter' after typing todo item
    var ESCAPE = 'Escape'; // if user pressed 'Esc' key
        ...
        switch (e.key) {
            case ENTER:
                ...
                if (editing && editing.length > 0) {
                    signal('SAVE')();   // invoke signal callback
                }
                var new_todo = document.getElementById('new-todo');
                
                // if there is a new todo, we `ADD` it
                if(new_todo.value.length > 0){
                    signal('ADD')();
                    new_todo.value = '';    //reset <input> for next todo item
